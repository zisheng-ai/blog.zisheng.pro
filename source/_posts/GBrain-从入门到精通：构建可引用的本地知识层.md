...
description: GBrain 不只是一个 RAG 工具，而是一层连接事实源、检索、Evidence、Citation 和 LLM 的知识基础设施。本文从基本概念、数据流、索引检索、云端与本地部署讲起，结合 Autoship 给出一套可落地的接入、验证和生产化方法。
categories:
  - [AI]
tags:
  - GBrain
...
我最近把 GBrain 接到了 Autoship 的 Rust Host 后面。接入之前，我对它的理解比较简单：把团队文档导进去，然后让 Agent 搜索。
...
代码 / Spec / ADR / 团队文档
   ...
   Evidence + Citation + Revision
          ↓
    Rust Host / Agent Context
          ↓
       Codex / Qoder
```

GBrain 负责“知识怎么被找到和组织”，Autoship 负责“谁能使用这些知识、作用于哪个 Workspace、最终执行是否成功”。两者不能混成一个边界。
...
Citation 是 Evidence 的可回溯地址。在 GBrain 接入 Autoship 的 MVP 中，我使用类似下面的形式：

```text
gbrain://default/autoship-gbrain知识架构与检索分层方案
```
...
Autoship 不允许拿旧 Revision 的索引结果冒充当前 Workspace 的知识。知识检索必须和当前 Session Pin、Workspace 和 Permission Scope 对齐。
...
GBrain 提供检索和知识组织能力，但不应该直接拥有组织发布权，也不应该替 Autoship 作出 Permission 或 Approval 决策。

## 在 Autoship 中的正确位置

Autoship 的执行借用本地 Codex 和 Qoder，因此 GBrain 最适合放在 Rust Host 后面：
...
2. Rust Host 可以先做 Workspace、Revision 和权限过滤。
3. GBrain 不可用时，可以降级为无知识检索，不阻断 Agent 主流程。

当前 Autoship 本地 MVP 的具体行为是：
...
```text
Autoship 已上线的云端 Control Plane 域名是什么？
如果知识库没有可靠证据，请回答不知道。
```
...
对 Autoship 来说，GBrain 的第一目标不是回答所有问题，而是让 Agent 在执行前拿到当前 Workspace 可用、来源明确、版本一致的知识。
