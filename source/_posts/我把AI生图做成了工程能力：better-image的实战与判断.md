---
title: 我把AI生图做成了工程能力：better-image的实战与判断
date: 2026-07-15 16:30:00
description: 这篇文章复盘我近期把 AI 生图用于产品视觉、角色设计、透明图标、macOS App Icon、动态壁纸与帧动画的真实过程，并介绍 better-image 如何把模型路由、Prompt 约束、后处理、交付规格和元数据沉淀成一套可复用的视觉工程能力。
categories:
  - [AI]
tags:
  - better-image
  ...
  - Sprite Animation
  - AI Infra
cover: /images/better-image-engineering-practice.webp
---
...
这也是我持续完善 [better-image](https://github.com/zisheng-ai/better-image) 的原因。它不是收藏 Prompt 的目录，而是一个面向 Agent 的视觉生成 skill：输入目标和交付类型，输出可以进入工程的图片、帧序列、元数据和平台产物。

## 一句话总结

**AI 生图要从“抽卡”走向工程化，关键不是把 Prompt 写得更长，而是把类型路由、规格约束、模型回退、后处理、质量检查和产物封装变成稳定流程。**

![better-image 能力概览](/images/better-image-capability-map.webp)

## 我最近到底拿它做了什么

我没有只扫描某一次会话，而是回看了近期 Codex 会话、项目资产和 better-image 自身的 Git 演进。真实需求大致分成这些类型：
...
![角色帧动画交付示例](/images/better-image-sprite-loop.gif)
...
better-image 现在默认使用 GPT Image 2，失败后归一化 Prompt 重试一次，再回退到 Gemini，最后才考虑 Doubao。
...
然后把稳定部分写进对应 reference，而不是继续依赖我的临场记忆。better-image 最近的演进也基本沿着这条路线：
...
![Argos 产品概念视觉案例](/images/better-image-argos-case.webp)
...
对我来说，better-image 最大的价值不是“让我更会画图”，而是让视觉生成开始拥有软件工程的确定性：知道该走哪条路径、失败时如何退、最后交付什么文件、下一次怎样复现。

从前端转向 Agent / AI Infra 后，我越来越相信一件事：**超级个体的优势不在于每件事都亲手做，而在于能把判断沉淀成系统，再让 Agent 把执行力拉满。** better-image 就是这套思路在视觉生成上的一次实践。
